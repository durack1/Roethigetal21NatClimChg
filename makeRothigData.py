...
actExpPair['ScenarioMIP'] = ['ssp119','ssp126','ssp245','ssp370','ssp434','ssp460','ssp534-over','ssp585']
    ...
    for count1,experimentId in enumerate(actExpPair[activityId]):
        ...
        for realm in variableId.keys():
            for count2,var in enumerate(variableId[realm]):
                print('mip:',mipEra,'actId:',activityId,'expId:',experimentId,'realm:',realm,'var:',var)
                searchPath = os.path.join(xmlPath,mipEra,activityId,experimentId,realm,frequency,var,'*.xml')
                ...
                if 'ssp534-over' in experimentId:
                    ...
                    print('experimentId:',experimentId,' - to _ fixed ',experimentId_)
                else:
                    experimentId_ = experimentId
                varName = '_'.join([mipEra,experimentId_,var])
                vars()[varName] = fileListTrim
                fileLists.extend([varName])
                del(searchPath,fileList,fileListTrim,varName,var) ; gc.collect()
...
#%% Declare file lists of problem data
badMods = ['.AWI-CM-1-1-MR.','.AWI-ESM-1-1-LR.','.bcc-csm1-1.','.bcc-csm1-1-m.',
           '.BCC-CSM2-MR.','.BCC-ESM1.','.CNRM-CM6-1-HR.','HadGEM3-GC31-MM',
           '.CMCC.CMCC-CM2-HR4.','.IITM-ESM.','CMIP6.CMIP.historical.NCAR.CESM2-WACCM-FV2.r3i1p1f1.mon.mrro.',
...
for count1, listy in enumerate(fileLists):
    ...
    for count2, filePath in enumerate(listVar):
        ...
        if any(x in filePath for x in badMods):
            strTxt = ' '.join([str(count2),'** Known grid issue with:',filePath.split('/')[-1],'skipping..**'])
            print(strTxt)
        ...
        outFMod = os.path.join(workDir,'ncs',dateNow,mipEra,experimentIdStartEndYrs,var,'modGrid')
        outFModId = os.path.join(outFMod,modId)
        woaId = '.'.join(['.'.join(filePath.split('/')[-1].split('.')[:-3]),'-'.join([str(startYr),str(endYr-1),'woaClim']),'nc'])
        outFWoa = os.path.join(workDir,'ncs',dateNow,mipEra,experimentIdStartEndYrs,var,'woaGrid')
        ...
        if experimentId == 'piControl':
            startYr = endYrChk-int(endYrCt.year) # Take climatological period from last year
